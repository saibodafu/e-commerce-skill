# e-commerce-skills｜电商工作流 Skill

这是一个用于沉淀电商实战工作流的 skill 包。当前已包含 3 个电商内容相关 skill，后续可以继续按同样结构追加新的电商 skill。

当前可用能力：

1. `ecommerce-content-wireframe`：从产品信息出发，梳理详情页内容结构、详情页文案，并生成可运行的手机长图 HTML 黑白线框稿。
2. `ecommerce-detail-page-visual-design`：在已有详情页原型图、包装图及其他可选资料的基础上，逐屏判断信息任务和视觉表达，推荐统一视觉风格，一屏一屏生成并确认，最后拼接成 750px 详情页长图。
3. `xhs-product-content-cards`：基于品类、产品、卖点、竞品或常见误区，生成小红书科普类、测评类、避坑类 2-4 张连续图文卡片方案和 AI 出图提示词。

## Skill 1：ecommerce-content-wireframe

电商详情页内容黑白线框稿。

### 安装

在 Claude Code、Codex、WorkBuddy 等支持 skills 的工具中运行：

```text
/install-skill saibodafu/e-commerce-skill/ecommerce-content-wireframe
```

### 使用场景

当你想梳理一个商品的电商详情页内容时使用。skill 会先要求你补充产品名、产品定位、目标人群、核心场景和核心卖点，再一步步生成详情页内容。

### 工作流程

第一阶段：先收集精准信息

用户先填写完整产品信息表。如果暂时不确定某一项，可以写“待定”，但产品名、目标人群、核心场景和核心卖点建议尽量具体。

第二阶段：再进入共创

产品信息确认后，skill 按固定顺序逐层推进：

1. 核心价值承诺层
2. 痛点锚定层
3. 卖点强化层
4. 使用场景层
5. 产品身份卡
6. 决策收口层

最终会先整理成完整的详情页内容文档，再生成可直接运行的 HTML 黑白线框稿。

### 示例输入

```text
使用 ecommerce-content-wireframe，帮我梳理一款儿童电解质水的详情页内容。

产品名：小K水
产品定位：儿童日常补水与电解质补充饮品
目标人群：3-12岁儿童家长
核心卖点：低糖、好喝、补充电解质、儿童友好
```

### 也可以这样说

```text
帮我梳理一个新品的电商详情页内容。
```

```text
用 ecommerce-content-wireframe，帮我基于完整产品信息生成详情页黑白线框稿。
```

```text
我有一个猫粮产品，帮我先整理详情页内容结构。
```

### 输出特点

- 先收集精准产品信息，再生成详情页内容
- 每一步都给多个选择，并指引用户选择、融合或补充信息
- 关键字段不足时先补齐，非关键细节不足时再做轻量假设并明确标注
- 核心卖点默认按“一个卖点一屏”拆成独立详情页屏幕，避免多个卖点挤在同一屏
- 不编造平台规则、销量、转化率、认证、竞品数据或客户案例
- 输出可以交给运营、内容、设计或商品团队继续执行

## Skill 2：ecommerce-detail-page-visual-design

电商详情页逐屏视觉设计与长图拼接。

### 安装

在 Claude Code、Codex、WorkBuddy 等支持 skills 的工具中运行：

```text
/install-skill saibodafu/e-commerce-skill/ecommerce-detail-page-visual-design
```

推荐在 Codex 下使用，并配合 ChatGPT Image 2 模型执行；这个组合对电商详情页视觉生成和中文文字呈现效果最好，更适合需要完整中文标题、卖点和长图文案的场景。

### 使用场景

当你已经有详情页原型图、包装图，并希望进入视觉设计阶段时使用。详情页文案、HTML 图、产品图、品牌规范、目标平台等资料不限，有则使用，没有则按现有信息推进并标注假设。

### 工作流程

skill 会按固定顺序推进：

1. 逐屏分析每一屏的信息任务
2. 确认每一屏的信息任务、视觉表达类型、核心视觉元素和风险点
3. 根据产品信息、品牌包装图和详情页文案，推荐 3 个适合电商详情页的视觉风格方向
4. 基于原型图中的每一屏文案、版式和信息层级，逐屏生成 AI 生图提示词，只生成提示词，不直接生成图片
5. 使用 ChatGPT Image 2 按顺序一屏一屏生成详情页设计
6. 每一屏修改和确认后再进入下一屏
7. 所有单屏确认后纵向拼接成 750px 长图
8. 检查漏屏、错序、重复、文字裁切和产品裁切
9. 输出最终长图和质检结果

### 示例输入

```text
使用 ecommerce-detail-page-visual-design，我已经有详情页原型图和包装图，帮我逐屏分析视觉表达，再按顺序一屏一屏生成，最后拼接成 750px 长图。
```

### 也可以这样说

```text
我有详情页 HTML 图和包装图，帮我判断每一屏适合怎么设计。
```

```text
帮我把确认好的单屏详情页按顺序拼成长图，宽度 750px。
```

### 输出特点

- 视觉设计流程中默认先分析、再定风格、再逐屏生成和确认
- 长图拼接后必须检查漏屏、错序、重复、文字裁切和产品裁切
- 卖点屏按“一个核心卖点一屏”检查和生成；如果原型图把多个卖点放在一屏，会先建议拆屏
- 原型图和包装图是启动必需材料，其他材料不限，有则使用

## Skill 3：xhs-product-content-cards

小红书带货图文卡片策划：把产品卖点、选购标准、竞品对比或用户误区，整理成适合小红书发布的 2-4 张连续图文卡片。

### 安装

在 Claude Code、Codex、WorkBuddy 等支持 skills 的工具中运行：

```text
/install-skill saibodafu/e-commerce-skill/xhs-product-content-cards
```

### 使用场景

当你不是要做电商详情页，而是要做小红书里的“可收藏、可转发、可带货”的图文内容时使用。

它适合解决这几类问题：

- 产品卖点很多，不知道怎么拆成小红书图文。
- 想做科普，但不想写成论文式长文。
- 想做测评对比，但不想变成硬广或踩竞品。
- 想做避坑内容，但不想制造焦虑或夸大风险。
- 已经有产品资料，希望直接生成每一页卡片的文案、版式和 AI 出图提示词。

### 三种图文模式

skill 会先判断内容更适合哪一种模式，也可以由你直接指定：

| 模式 | 适合什么时候用 | 输出重点 |
|---|---|---|
| 科普类 | 用户需要看懂品类、成分、参数、机制或选购标准 | 把复杂信息讲清楚，给出判断标准 |
| 测评类 | 用户在多个产品、价位、方案之间纠结 | 用同一套维度对比，让目标产品优势自然出现 |
| 避坑类 | 用户容易被价格、包装、概念话术或错误场景误导 | 拆常见误区，给出正确选择标准 |

### 工作流程

skill 会按固定顺序推进：

1. 提取用户输入，补齐轻量假设
2. 判断内容模式：科普类、测评类或避坑类
3. 判断是否需要联网核验公开资料
4. 设计第 1 张封面图和第 2-4 张拆解页
5. 输出每页标题、副标题、中间主体、底部总结、版式建议和风险提醒
6. 输出逐页 AI 出图提示词

### 你需要提供什么

最少提供这几项就能开始：

```text
品类/产品：
目标用户：
图文主题：
核心卖点或用户痛点：
希望传递的判断标准或倾向性结论：
品牌露出需求：弱露出 / 自然带出 / 可不出现
```

如果是测评类，建议补充自家产品、对比产品、测评维度和希望放大的优势。

如果是避坑类，建议补充常见坑点、用户容易误判的选择点和希望用户记住的购买理由。

### 示例输入：避坑类

```text
使用 xhs-product-content-cards，帮我做一组小红书避坑类图文。

品类：儿童电解质水
目标用户：3-12 岁孩子家长
避坑主题：儿童电解质水别乱选
核心卖点：低糖、口味友好、适合日常补水场景
品牌露出需求：自然带出
```

### 示例输入：测评类

```text
使用 xhs-product-content-cards，帮我做 3 张小红书测评图文。

品类：通勤保温杯
目标用户：每天带咖啡上班的女生
测评主题：3 款通勤保温杯怎么选
测评维度：容量、重量、密封性、颜值、清洗难度
自家产品：A 品牌轻量保温杯
希望放大的优势：轻、好看、不漏、适合通勤包
对比产品：平价款、高端款、大容量款
品牌露出需求：弱露出
```

### 也可以这样说

```text
帮我做 3 张小红书科普卡片，主题是新手怎么选猫粮。
```

```text
做一组小红书测评图文，比较 3 款同类产品，突出我家产品更适合通勤人群。
```

```text
把这个产品卖点转成小红书避坑清单风格的 3:4 图文提示词。
```

### 输出特点

- 默认输出 2-4 张，优先 3 张：第 1 张封面，第 2-4 张拆解页。
- 每张只讲一个核心信息：一个知识点、一个测评维度或一个避坑标准。
- 每页都会给出主标题、副标题、中间主体、底部总结、画面布局、视觉元素和风险提醒。
- 最后输出逐页 AI 出图提示词，适合继续交给图像模型生成 3:4 小红书卡片。
- 不伪装真人实测，不编造检测数据、竞品参数、行业榜单或平台规则
- 涉及健康、母婴、营养、安全、功效或最新产品信息时，先联网核验并标注来源和时间

## 项目结构

```text
.
├── AGENTS.md
├── commands/
│   └── ecommerce.md
├── skills/
│   ├── ecommerce-content-wireframe/
│   │   └── SKILL.md
│   ├── ecommerce-detail-page-visual-design/
│   │   └── SKILL.md
│   └── xhs-product-content-cards/
│       └── SKILL.md
├── .claude-plugin/
└── .codex-plugin/
```

## 赛博大福

欢迎关注公众号“赛博大福”。

用 AI 把营销重干一遍。
